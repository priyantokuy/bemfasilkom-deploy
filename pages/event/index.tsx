          ...
          {EventsList.length > 0 ? (
            <>
                  ...
                  return (
                      <a key={idx} href={Events.direct_link} target='_blank' rel="noreferrer" className='group card hover:translate-y-[-10px] transition-all overflow-hidden cursor-pointer bg-gradient-cust-orange2 hover:bg-none rounded-md border-2 bg-pastel pb-4 box-border drop-shadow p-[.1rem] w-fit min-[550px]:w-[80%] sm:max-w-none sm:w-[45%] lg:w-fit'>
                        <article className='w-fit lg:w-[20rem] flex flex-col flex-wrap lg:justify-center '>
                          <p className={`${Date.parse(Events.end_date) > Date.now() ? 'bg-gradient-cust-orange2' : 'bg-red-500'} text-center text-white w-fit absolute right-0 top-0 p-2 rounded-sm`} >{Date.parse(Events.end_date) > Date.now() ? "Open" : "Closed"}</p>
                          <img className='rounded-t-xl object-cover h-[180px]' src={Events.event_image ? API_URL + Events.event_image.url : 'placeholder_image_url'} alt="Events" />
              ...
              {EventsList.length < EventsCount ? (
                <button
                  className="block border-2 border-tangerine hover:bg-gradient-cust-orange2 text-black font-medium py-2 px-4 rounded-full mt-[60px] mx-auto"
